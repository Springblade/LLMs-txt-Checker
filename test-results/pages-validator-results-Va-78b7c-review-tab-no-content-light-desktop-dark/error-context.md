...
- Name: pages\validator-results.spec.ts >> Validation Results Page >> Result — found, Preview tab (no content), light
- Location: e2e\pages\validator-results.spec.ts:259:7
...
```ts
  170 |   test("Result — invalid_response error, dark", async ({ page }) => {
  171 |     await setDarkMode(page);
  172 |     await mountValidationResults(page, {
  173 |       found: false,
  174 |       message: "Invalid response received",
  175 |       errorCode: "invalid_response",
  176 |       errors: [],
  177 |       warnings: [],
  178 |     });
  179 | 
  180 |     await pageSnapshot(page, "validator-results", "invalid-response");
  181 |   });
  182 | 
  183 |   test("Result — http_error, light", async ({ page }) => {
  184 |     await setLightMode(page);
  185 |     await mountValidationResults(page, {
  186 |       found: false,
